...
// --- Profiler ---
// (Код профайлера оставлен без изменений из предыдущих версий для отладки, если нужно)
class Profiler {
public:
    struct Stats {
    ...
    void printStats() {
        ...
        qDebug() << "====================================\n";
        QFile file("profiler_stats_trade.txt"); // Отдельный файл для удобства
...
private slots:
    ...
    void processMessage(const QString& message) {
        // PROFILE_SCOPE("DataWorker::processMessage"); // Отключаем для высокой частоты
        if (!running) return;
        ...
        // Изменение: Парсим данные стрима @trade
        QString symbol = data["s"].toString(); // "BTCUSDT"
        QString currency = symbol.left(symbol.length() - 4).toUpper(); // "BTC"

        // Цена сделки
        double price = data["p"].toString().toDouble();
        // Время сделки в миллисекундах
        double timestamp = data["T"].toDouble() / 1000.0; // Преобразуем в секунды с дробной частью
        // Объем сделки (может понадобиться для взвешенной цены)
        // double quantity = data["q"].toString().toDouble();
        ...
        if (now - last_report_time >= 10) {
            ...
            qDebug() << "WebSocket TRADE messages per second:";
            for (const QString& cur : CURRENCIES) {
        ...
        // Эмитируем сигнал с данными о сделке
...
private:
    void connectWebSocket() {
        ...
        // Изменение: Подписываемся на @trade стримы
        QStringList streams;
        for (const QString& cur : CURRENCIES) {
            streams << QString("%1usdt@trade").arg(cur.toLower()); // btcusdt@trade
        }
...
// --- ChartConfig ---
// (Код ChartConfig оставлен без изменений)
...
public:
    ChartConfig(QWidget* parent = nullptr) : QDialog(parent) {
        ...
        cacheSize->setRange(100, 864000); // Увеличиваем максимум для высокочастотных данных
...
public:
    DynamicSpeedometer(const QString& currency, QWidget* parent = nullptr)
        ...
        minInit(0.9995), maxInit(1.0005), volatilityWindow(1000), // Увеличиваем окно волатильности
        minFactor(1.00001), maxFactor(0.99999), // Более плавные границы
        maxPoints(500), sampleMethod(0), cacheSize(864000), // Увеличиваем кэш
        ...
        animation->setDuration(900); // Очень быстрая анимация для высокочастотных данных
        connect(animation, &QPropertyAnimation::valueChanged, this, [this]() {
            if (mode == "speedometer") {
                update(); // Перерисовываем спидометр на каждом шаге анимации
            }
        });
        ...
        // --- Таймеры ---
        // 1. Таймер для ограничения частоты перерисовки графиков (60 FPS)
        ...
        // 2. Таймер для редкого пересчета данных графика (4 раза в секунду)
        ...
        cacheUpdateTimer->start(250); // Пересчитываем кэш каждые 250 мс
    ...
    // Этот метод вызывается очень часто
    void updateData(double price, double timestamp, double btcPrice = 0) {
        // PROFILE_SCOPE(QString("DynamicSpeedometer::updateData_%1").arg(currency));

        // 1. Обновляем историю сделок
        ...
        } else if (btcPrice > 0) { // Защита от деления на 0
        ...
        // 2. Обновляем волатильность и границы на основе последних данных
        updateVolatility();
        updateBounds(price);

        // 3. Обновляем анимацию спидометра
        double scaled = 50;
        ...
        // 4. Помечаем, что данные изменились и нужна перерисовка графика
        dataNeedsRedraw = true;
        // НЕ вызываем update() напрямую здесь для графика
    }
...
protected:
    void paintEvent(QPaintEvent*) override {
        // PROFILE_SCOPE(QString("DynamicSpeedometer::paintEvent_%1").arg(currency));
...
private slots:
    ...
    void setTimeScale(const QString& scale) {
        ...
        if (timeScales.contains(scale)) {
            currentScale = scale;
            cacheChartData(); // Запрашиваем немедленный пересчет кэша
    ...
    void onRenderTimeout() {
        ...
        // Перерисовываем график только если это необходимо
        if (dataNeedsRedraw && (mode == "line_chart" || mode == "btc_ratio")) {
...
private:
    ...
    void cacheChartData() {
        ...
        // Этот метод вызывается редко по таймеру cacheUpdateTimer
    ...
    void updateBounds(double price) {
        ...
        // Более плавное изменение границ
        ...
        double epsilon = range * 0.00001; // Динамический эпсилон
    ...
    void drawSpeedometer(QPainter& painter) {
        ...
        if (cachedMinVal.has_value() && cachedMaxVal.has_value() && !history.empty()) {
            double currentPrice = history.back().second; // Цена последней сделки
            painter.setFont(QFont("Arial", 21, QFont::Bold));
            // Используем QLocale для правильного форматирования чисел
        ...
        painter.drawText(QRect(1, 1, w - 10, 20), Qt::AlignLeft | Qt::AlignTop,
                         QString("Volatility: %1%").arg(volatility, 0, 'f', 4));
    }
    ...
    QTimer* cacheUpdateTimer;  // Таймер для редкого обновления кэша (250ms)
...
public:
    CryptoDashboard() : btcPrice(0) {
        ...
        setWindowTitle("Crypto Dashboard (TRADE Stream)");
        resize(1600, 900);
...
private slots:
    void handleData(const QString& currency, double price, double timestamp) {
        ...
        if (speedometers.contains(currency)) {
            ...
            // Используем QueuedConnection для безопасной передачи в GUI поток
